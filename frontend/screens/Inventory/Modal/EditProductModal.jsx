...
// Adjust this to wherever your backend serves stored images from
const IMAGE_BASE_URL = 'https://your-api-domain.com/storage/'

const EditProductModal = ({ visible, product, onClose, onProductUpdated }) => {
  const { t } = useLanguage()
  ...
  // Image states
  const [imageUri, setImageUri] = useState(null)       // existing remote image or newly picked local uri
  const [imageAsset, setImageAsset] = useState(null)    // set only when a NEW image is picked
  const [imageRemoved, setImageRemoved] = useState(false) // true if user explicitly removed existing image
  ...
  // Populate data when modal opens with a specific product
  useEffect(() => {
    if (visible && product) {
      setName(product.name || '')
      setCategoryId(product.category_id || product.category?.id || null)
      setSelectedCategoryName(product.category?.name || 'Select a category')
      setDescription(product.description || '')
      setPrice(product.price != null ? product.price.toString() : '')
      setBuying_price(product.buying_price != null ? product.buying_price.toString() : '')
      setStock(product.stock != null ? product.stock.toString() : '')
      setUnit(product.unit || 'pcs')
      setAllowDecimal(!!product.allow_decimal)

      setImageAsset(null)
      setImageRemoved(false)
      setImageUri(product.image ? `${IMAGE_BASE_URL}${product.image}` : null)

      // Match the saved currency code to the full name from the JSON file
      if (product.currency) {
        setCurrencyCode(product.currency)
        const matchedCurrency = currenciesData.find(c => c.currency_code === product.currency)
        setSelectedCurrencyName(matchedCurrency ? matchedCurrency.currency : product.currency)
      } else {
        setCurrencyCode(null)
        setSelectedCurrencyName('Select currency')
      }

      // Safe check and conversion for parsing backend date formats
      if (product.expiry_date) {
        const normalizedDateStr = product.expiry_date.replace(' ', 'T')
        setExpiryDate(new Date(normalizedDateStr))
      } else {
        setExpiryDate(null)
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Multipart only for a real image upload on a device; JSON otherwise so
      // it works on the web (where a manual multipart Content-Type breaks).
      if (imageAsset && Platform.OS !== 'web') {
        ...
        response = await axiosClient.put(`/products/${product.id}`, payload)
      }
  ...
  return (
    <Modal visible={visible} transparent={true} animationType="fade">
        ...
        <View style={styles.overlay}>
          ...
          >
            <View style={styles.modalContainer}>
              <Text style={styles.heading}>{t('prod.editTitle')}</Text>
              ...
              <View style={styles.buttonContainer}>
                ...
                >
                  {isSaving
                    ...
                    : <Text style={styles.saveButtonText}>{t('prod.update')}</Text>
                  }
                </TouchableOpacity>
              </View>
            </View>
      ...
      {/* Category Dropdown Search Modal */}
      ...
      {/* Currency Dropdown Search Modal */}
